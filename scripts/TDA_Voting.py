...
def first_read(org,df):
    """ Esta función devuelve los reads que han mapeado
    al organismo org. Un read que mapea multiples veces 
    a un mismo organismo se cuenta solo una vez.
    """
    #return set(df[df["Organims"]==org].Qname)
...
def get_read_from_organism(orgs):
    """Esta función obtiene la intersección de los reads
    que mapean a una lista de organismos.
    """
    inter=orgs[0]
    for org in orgs:
...
def ranking_function_cech(df_mapping):
    # Organismos
    #organisms=list(df_mapping["Organims"].unique())
    ...
    #lista_rankings = [rank_organisms, rank_org_normalized]
    #return rank_organisms
...
def getIDnames2(fname):
    x = []
    x.append(fname.split("/")[-1].split("_")[-1].split(".")[0])
    return x
